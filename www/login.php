...
if (@$_GET['action'] == 'login') {
	...
	//到数据库去验证
	if (!!$_rows = _fetch_array("SELECT tg_username,tg_uniqid,tg_level FROM tg_user WHERE tg_username='{$_clean['username']}' AND tg_password='{$_clean['password']}'  LIMIT 1")) {
		//登录成功后，记录登录信息
...
<div id="login">
	...
	<form method="post" name="login" action="login.php?action=login">
		<dl>
			<dt></dt>
			<dd>用 户 名：<input type="text" name="username" class="text" placehold="username"/></dd>
			<dd>密　　码：<input type="password" name="password" class="text"  placehold="password"/></dd>
